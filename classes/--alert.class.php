...
//                        }
		    ...
		    $badges = ',"badge":{"badgeId":"'.$valss[$i]['badges']['badge_id'].'","badgeName":"'.$valss[$i]['badges']['badge_name'].'","badgeDescription":"'.$valss[$i]['badges']['badge_description'].'",
			"badgeThumbImageURL":"'.$valss[$i]['badges']['badgeThumbImageURL'].'","badgeImageURL":"'.$valss[$i]['badges']['badgeImageURL'].'","badgeEarned":"'.$valss[$i]['badges']['badgeEarned'].'","venueId":"'.$valss[$i]['badges']['venueId'].'","badgeUnlockedAtVenue":"'.$valss[$i]['badges']['badgeUnlockedAtVenue'].'","badgeUnlockedAtTime":"'.$valss[$i]['badges']['badgeUnlockedAtTime'].'"}';
		    $tesmp = '';//for unnecessary splid coming in response
                ...
                $str_temp = '{
                        ...
                        "alertDate":"' . trim($alertDate) . '",
                        "alertDescription":"' . trim($alertDescription) . '",
                        "alertImageUrl":"' . trim($alertImageUrl) . '",
    ...
    public function alerts_update($response_message, $xmlrequest) {
        ...
        if ($updateAlert > 0) {
           ...
           ' . $response_str . '
           "AlertsUpdate":{
                "errorCode":"' . $return_codes["AlertsUpdate"]["SuccessCode"] . '",
                "errorMsg":"' . $return_codes["AlertsUpdate"]["SuccessDesc"] . '",
                "AlertUpdated":"' . $updateAlert['msgType'] . ' is updated successfully' . '"
                }
        }';
        } else {
            $response_mess = '
	...
	$input = isset($hotpressAlert['parent'][0]['body']) && ($hotpressAlert['parent'][0]['body']) ? $hotpressAlert['parent'][0]['body'] : NULL;
	    ...
	    $str_temp = '{
            "authorID":"' . $hotpressAlert['parent'][0]['mem_id'] . '",
            "postId":"' . $hotpressAlert['parent'][0]['id'] . '",
            "authorName":"' . $hotpressAlert['parent'][0]['profilenam'] . '",
            "authorImgURL":"' . $hotpressAlert['parent'][0]['photo_b_thumb'] . '",
            "likeCount":"' . $hotpressAlert['parent'][0]['likecount'] . '",
            "postBody":"' . $hotpressAlert['parent'][0]['body'] . '",
            "postType":"' . $postType . '",
            "uploadedImage":"' . $hotpressAlert['parent'][0]['image_link'] . '",
            "width_image_link":"' . $width_image_link . '",
            "height_image_link":"' . $height_image_link . '",
            "link_url":"' . $hotpressAlert['parent'][0]['link_url'] . '",
            "postTimestamp":"' . $date . '",
            "youtubeLink":"' . $hotpressAlert['parent'][0]['youtubeLink'] . '",
            "link_image":"' . $hotpressAlert['parent'][0]['link_image'] . '",
            "width_link_image":"' . $width_link_image . '",
            "height_link_image":"' . $height_link_image . '",
            "postVia":"'.$postVia.'",
	    "commentsCount": "' . $hotpressAlert['parent']['totalCount'] . '",
            "currentCommentsCount": "' . $hotpressAlert['parent']['currentCount'] . '"
        }';
                    $strp = $strp . $str_temp;
//for child count
		    if ($count > 0) {
		...
		$input = isset($hotpressAlert['child'][$i]['body']) && ($hotpressAlert['child'][$i]['body']) ? $hotpressAlert['child'][$i]['body'] : NULL;
		    ...
		    $str_temp = '{
            "commentId":"' . $hotpressAlert['child'][$i]['id'] . '",
            "authorID":"' . $hotpressAlert['child'][$i]['mem_id'] . '",
            "authorProfileImgURL":"' . $hotpressAlert['child'][$i]['photo_b_thumb'] . '",
            "authorName":"' . $hotpressAlert['child'][$i]['profilenam'] . '",
            "commentBody":"' . $hotpressAlert['child'][$i]['body'] . '",
            "commentType":"' . $postType . '",
            "commentTimestamp":"' . $date . '",
	    "postVia":"'.$postViaChild.'"
        }';
		    $str = $str . $str_temp;
		    $str = $str . ',';
		}
...
//        print_r($commentAlert);
	$count = $commentAlert['child']['count'];
        ...
        if (!empty($commentAlert)) {
		    ...
		    $str_temp = '{
            "postId":"' . $commentAlert['parent'][0]['tst_id'] . '",
            "authorID":"' . $commentAlert['parent'][0]['mem_id'] . '",
            "authorProfileImgURL":"' . $commentAlert['parent'][0]['photo_b_thumb'] . '",
            "authorName":"' . $commentAlert['parent'][0]['profilenam'] . '",
            "gender":"' . $commentAlert['parent'][0]['gender'] . '",
            "profileType":"' . $commentAlert['parent'][0]['profile_type'] . '",
            "postText":"' . trim(preg_replace('/\s+/', ' ', $commentAlert['parent'][0]['testimonial'])) . '",
            "postType":"' . $postType . '",
            "uploadedImage":"' . $commentAlert['parent'][0]['image_link'] . '",
            "width_image_link":"' . $width_image_link . '",
            "height_image_link":"' . $height_image_link . '",
            "link_url":"' . $commentAlert['parent'][0]['link_url'] . '",
            "youtubeLink":"' . $commentAlert['parent'][0]['youtubeLink'] . '",
            "link_image":"' . $commentAlert['parent'][0]['link_image'] . '",
            "width_link_image":"' . $width_link_image . '",
            "height_link_image":"' . $height_link_image . '",
            "postTimestamp":"' . $date . '",
            "postVia":"'.$postVia.'",
            "commentsCount":"' . $commentAlert['parent']['totalCount'] . '"
            "currentCommentsCount":"' . $commentAlert['parent']['totalCount'] . '"
        }';
		}
                    $strp = $strp . $str_temp;
...
//print_r($commentAlert['child']);
		    ...
		    $str_temp = '{
            "postId":"' . $commentAlert['child'][$i]['tst_id'] . '",
            "authorID":"' . $commentAlert['child'][$i]['mem_id'] . '",
            "authorProfileImgURL":"' . $commentAlert['child'][$i]['photo_b_thumb'] . '",
            "authorName":"' . $commentAlert['child'][$i]['profilenam'] . '",
            "gender":"' . $commentAlert['child'][$i]['gender'] . '",
            "profileType":"' . $commentAlert['child'][$i]['profile_type'] . '",
            "postText":"' . trim(preg_replace('/\s+/', ' ', $commentAlert['child'][$i]['testimonial'])) . '",
            "postType":"' . $postType . '",
            "postTimestamp":"' . $date . '",
            "postVia":"'.$postVia.'"
        }';
    ...
    public function displayPhotoTagAlert($response_message, $xmlrequest) {
		...
		$userinfo['photo']['photo_owner_id'] = (isset($userinfo['photo']['album_owner_id']) && !empty($userinfo['photo']['album_owner_id'])) ? $userinfo['photo']['album_owner_id'] : NULL;
			 ...
			 $str = '
             "photo":"' . $userinfo['photo']['photo_mid'] . '",
			 "photo_owner_id":"' . $userinfo['photo']['photo_owner_id'] . '",
             "width":"' . $width . '",
             "height":"' . $height . '",
             "subject":"' . $userinfo['message_info']['subject'] . '",
             "date":"' . date("F j, Y", $userinfo['message_info']['date']) . '",
';
            $response_mess = '
               {
   ' . response_repeat_string() . '
