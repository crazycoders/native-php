...
//                $query_hotpress_like = "SELECT `hotpressid` AS parentid FROM `likehot` where `hotpressid` In (SELECT `id` FROM `bulletin` where  date  >= '" . (time() - (24 * 60 * 60)) . "') GROUP BY `hotpressid` ORDER BY count( `hotpressid` ) DESC";
		...
		if ((mysql_num_rows($exe_hotpress_like) > 0)) {
			...
			if ((mysql_num_rows($hotpressQuery) > 0)) {
			    $str = $this->getHotpressResponse($hotpressQuery);
			} else {
			    $str = '';
			}
		    }
		...
		$query = "SELECT DISTINCT b.appearance_id,b.photo_album_id,b.eventcmmnt_id,b.id, b.mem_id, b.subj, b.body,
			b.date, b.parentid, b.from_id, b.visible_to,b.image_link,b.post_via,b.youtubeLink,b.link_image,b.link_url,b.auto_genrate_text";
...
//for todays top
    ...
    function getHotpressResponse($hotpressQuery) {
	$postcount = 0;
	$str = '';
	$app_comment = '';
	$i=0;
	$hotpress=array();
	//$hotpressQuery = mysql_query($query);
	//if ((mysql_num_rows($hotpressQuery) > 0)) {
	while ($appResult = mysql_fetch_array($hotpressQuery, MYSQL_ASSOC)) {
	    ...
	    $hotpress[$i]['user_like'] = $this->get_user_like($hotpress[$i]['id']);
	    ...
	    $hotpress[$i]['profileImageUrl'] = isset($tmp_userinfor_array['profileImageUrl']) && (strlen($tmp_userinfor_array['profileImageUrl']) > 7) && ($tmp_userinfor_array['is_facebook_user'] == 'y' || $tmp_userinfor_array['is_facebook_user'] == 'Y') ? $tmp_userinfor_array['profileImageUrl'] : ((isset($tmp_userinfor_array['profileImageUrl']) && (strlen($tmp_userinfor_array['profileImageUrl']) > 7)) ? $this->profile_url . $tmp_userinfor_array['profileImageUrl'] : $this->profile_url . default_images($tmp_userinfor_array['gender'], $tmp_userinfor_array['profile_type']));
	    ...
	    $hotpress[$i]['link_image'] = isset($appResult['link_image']) && (strlen($appResult['link_image']) > 7) ? $this->profile_url . $appResult['link_image'] : "";
...
function get_appearance_id($Id) {
	...
	if (!empty($result)) {
		...
		$appearance_id['badge_image'] = isset($result['badge_img']) && ($result['badge_img']) ? $result['badge_img'] : NULL;
	    $appearance_id['bottel_image'] = isset($result['badge_bottle_img']) && ($result['badge_bottle_img']) ? $result['badge_bottle_img'] : NULL;
	} else {
//echo "SELECT announce_arrival.venue_id,members.profilenam from announce_arrival,members WHERE announce_arrival.id='$Id' AND announce_arrival.user_id=members.mem_id ";
...
/*	echo "SELECT announce_arrival.venue_id, members.profilenam,bottel_alert.badge_type_id,badges.badge_img,badges.badge_bottle_img FROM members,announce_arrival LEFT JOIN bottel_alert ON announce_arrival.id = bottel_alert.app_id LEFT JOIN badges ON bottel_alert.badge_type_id=badges.badge_id WHERE announce_arrival.id = '106'  and announce_arrival.venue_id = members.mem_id";*/
	    ...
	    $appearance_id['badge_image'] = isset($query1['badge_img']) && ($query1['badge_img']) ? $query1['badge_img'] : NULL;
	    $appearance_id['bottel_image'] = isset($query1['badge_bottle_img']) && ($query1['badge_bottle_img']) ? $query1['badge_bottle_img'] : NULL;
	}
