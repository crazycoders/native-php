...
$appNotfn = array();
	...
	if (empty($resultBottelalert)) {
	    ...
	    $sqlinsertintohotpress = "INSERT INTO bulletin( `id` , `mem_id` ,  `body` , `date` ,`auto_genrate_text` )
					     VALUES ( NULL , '" . $uid . "',  'has just earned the Trendsetter Bottles. Congrats on starting something cool!', '" . time() . "','Y' )";
	...
	if ($result['count'] > 0) {
		...
		if ($resultBottelalert['count'] > 0) {
		  ...
		  Value('Las Vegas','2','".$announce_id."','" . $uid . "','Congrats! You have just earned the Viva Las Vegas bottle','".date('Y-m-d H:i:s',time())."','$venue_id','$time')";
		    ...
		    $proname = show_from_members($uid);
		    $sqlinsertintohotpress = "INSERT INTO bulletin( `id` , `mem_id` ,  `body` , `date` ,`auto_genrate_text` )
		                                 VALUES ( NULL , '" . $uid . "',  'has just earned the Viva Las Vegas Bottles. If you dont hear from them in 2 days, dont worry, Vegas was good to them and recovery has started.', '" . time() . "','Y' )";
		    execute_query($sqlinsertintohotpress, true, "insert");
		}
	    }
	}
	...
	if ($statusLA == 1) {
		...
		$proname = show_from_members($uid);
		$sqlinsertintohotpress = "INSERT INTO bulletin( `id` , `mem_id` ,  `body` , `date` ,`auto_genrate_text` )
		                                 VALUES ( NULL , '" . $uid . "',  'has just earned the Tinsel Town Bottles. Red Carpet events, celebrity sightings etc.. Its going to be a fun time.', '" . time() . "','Y' )";
		execute_query($sqlinsertintohotpress, true, "insert");
	    }
	}
    }
	...
	if ($statusMiami == 1) {
		...
		$proname = show_from_members($uid);
		$sqlinsertintohotpress = "INSERT INTO bulletin( `id` , `mem_id` ,  `body` , `date`,`auto_genrate_text` )
		     VALUES ( NULL , '" . $uid . "','has just earned the Miami Beach Bottles. Look out for some amazing photos and a new tan.', '" . time() . "' , 'Y')";
		execute_query($sqlinsertintohotpress, true, "insert");
	    }
	}
    }
	...
	if ($statusNewYork == 1) {
		...
		$proname = show_from_members($uid);
		$sqlinsertintohotpress = "INSERT INTO bulletin( `id` , `mem_id` ,  `body` , `date` ,`auto_genrate_text` )
		    VALUES ( NULL , '" . $uid . "',  'has just earned the NYC Bottles. They could soon be hanging with the It crowd.', '" . time() . "','Y' )";
		execute_query($sqlinsertintohotpress, true, "insert");
	    }
	}
    }
...
//    $statusNewYork = 1;
	...
	if (empty($resultBottelalert)) {
	    ...
	    $proname = show_from_members($uid);
	    $sqlinsertintohotpress = "INSERT INTO bulletin( `id` , `mem_id` ,  `body` , `date` ,`auto_genrate_text` )
		    VALUES ( NULL , '" . $uid . "',  'has just earned the Socialite Bottles. They are part of the It crowd now. They might already have an agent.', '" . time() . "','Y' )";
	    execute_query($sqlinsertintohotpress, true, "insert");
	}
    } else {
	$inEachCity = 0;
...
//for nightsite venue
		    ...
		    $proname = show_from_members($uid);
		    $sqlinsertintohotpress = "INSERT INTO bulletin( `id` , `mem_id` ,  `body` , `date` ,`auto_genrate_text` )
		    VALUES ( NULL , '" . $uid . "',  'is now Ambassador for " . getname($venue_id) . ".', '" . time() . "','Y' ),( NULL , '" . $uid . "',  '" . $proname['profilenam'] . "  has ousted (" . getname($getPreAmbassadrList[0]['mem_id']) . ") and is now the Ambassador for (" . getname($venue_id) . ").', '" . time() . "','Y' )";
		    execute_query($sqlinsertintohotpress, true, "insert");
		} else {
...
//for nightsite venue
			...
			$proname = show_from_members($uid);
			$sqlinsertintohotpress = "INSERT INTO bulletin( `id` , `mem_id` ,  `body` , `date` ,`auto_genrate_text` )
			VALUES ( NULL , '" . $uid . "',  'has just earned the Ambassador Bottles. Treat them well. They might be the key to getting into hot events', '" . time() . "','Y' )";
			execute_query($sqlinsertintohotpress, true, "insert");
		    }
		}
	    } else {
...
//for nightsite venue
	...
	if ($result7day[0]['cnt'] >= 14) {
		...
		$proname = show_from_members($uid);
		$sqlinsertintohotpress = "INSERT INTO bulletin( `id` , `mem_id` ,  `body` , `date` ,`auto_genrate_text` )
					   VALUES ( NULL , '" . $uid . "',  'has just earned the Rock Star Bottles. This is no joke. They now deserve your respect!', '" . time() . "' ,'Y')";
		execute_query($sqlinsertintohotpress, true, "insert");
	    }
	} else {
	    $day14 = 0;
...
//    $sqlEntourage = "SELECT DISTINCT a.* FROM announce_arrival a INNER JOIN network n ON (n.mem_id = a.user_id) WHERE ('$uid' IN (n.frd_id, a.user_id)) AND a.venue_id='$venue_id' AND a.date=CURDATE() GROUP BY a.user_id ORDER BY a.date ASC";
	...
	if (empty($resultBottelalert)) {
	    ...
	    $proname = show_from_members($uid);
	    $sqlinsertintohotpress = "INSERT INTO bulletin( `id` , `mem_id` ,  `body` , `date`,`auto_genrate_text` )
					   VALUES ( NULL , '" . $uid . "',  'has just earned the Entourage Bottles. Rollin deep is normal for VIPs!', '" . time() . "','Y' )";
	    execute_query($sqlinsertintohotpress, true, "insert");
	}
    } else {
	$entourage = 0;
...
//    $sql = "SELECT DISTINCT(venue_id) FROM announce_arrival WHERE user_id = '" . $uid . "' ";
	...
	if (empty($resultBottelalert)) {
	    ...
	    $proname = show_from_members($uid);
	    $sqlinsertintohotpress = "INSERT INTO bulletin( `id` , `mem_id` ,  `body` , `date` ,`auto_genrate_text` )
					   VALUES ( NULL , '" . $uid . "',  'has just earned the Party Animal Bottles. Can you keep up with them?', '" . time() . "' ,'Y')";
	    execute_query($sqlinsertintohotpress, true, "insert");
	}
    } else {
	$partyAnimal = 0;
	...
	if (empty($resultBottelalert)) {
	    ...
	    $proname = show_from_members($uid);
	    $sqlinsertintohotpress = "INSERT INTO bulletin( `id` , `mem_id` ,  `body` , `date` ,`auto_genrate_text` )
							   VALUES ( NULL , '" . $uid . "',  'has just earned the Party Crasher Bottles. They obviously have no problem getting in', '" . time() . "','Y' )";
	    execute_query($sqlinsertintohotpress, true, "insert");
	}
    } else {
	$day24 = 0;
	...
	if (empty($resultBottelalert)) {
	    ...
	    $proname = show_from_members($uid);
	    $sqlinsertintohotpress = "INSERT INTO bulletin( `id` , `mem_id` ,  `body` , `date` ,`auto_genrate_text` )
						   VALUES ( NULL , '" . $uid . "',  'has just earned the Jetsetter Bottles. This global party hopper is on another level.', '" . time() . "', 'Y' )";
	    execute_query($sqlinsertintohotpress, true, "insert");
	}
    } else {
	$jetsetter = 0;
